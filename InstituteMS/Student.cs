...
namespace InstituteMS
{
    public partial class Student : Form
    {
        ...
        StudentAttendence stuatten = new StudentAttendence();
            ...
            if (StuAvideoPlayer.GetCurrentVideoFrame() != null)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        string check=stuatten.checksave(int.Parse(StuAstuid.Text),DateTime.Now.Date.ToString("MMMM"));
                        if (status !=null)
                        {                          
                            ...
                            if (check == null)
                            {
                                int maxid = db.GetLastID("SELECT MAX(attenID) FROM ims.attendance");
                                stuatten.Month = DateTime.Now.Date.ToString("MMMM");
                                stuatten.Year = DateTime.Now.Year.ToString();
                                stuatten.save(maxid + 1, int.Parse(StuAstuid.Text), int.Parse(StuAclassid.Text));
                                checklbl.Text= "Student Attendance Save";
        ...
        private void StuRid_KeyPress(object sender, KeyPressEventArgs e)
        {
            v.NumTextBox(e);
        }

        private void StuRclassid_KeyPress(object sender, KeyPressEventArgs e)
        {
            v.NumTextBox(e);
        }

        private void StuPid_KeyPress(object sender, KeyPressEventArgs e)
        {
            v.NumTextBox(e);
        }
